...
app.get('/', (req, res) => {
  let tmpCocktails = cocktails;
  if (req.query.alcohol) {
    tmpCocktails = tmpCocktails.filter((item) => item.contains.indexOf(req.query.alcohol) < 0);
  }

  res.render('home', { cocktails: tmpCocktails, alcoholList });
});
